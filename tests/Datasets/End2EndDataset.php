...
dataset('defaultTemplates', [
    '_base' => [
        'chatTemplate' => "{% for message in messages %}{{'' + message['role'] + '\\n' + message['content'] + '' + '\\n'}}{% endfor %}{% if add_generation_prompt %}{{ 'assistant\\n' }}{% endif %}",
        'data'          => [
...
dataset('customTemplates', [
    ...
    'HuggingFaceH4/zephyr-7b-gemma-v0.1' => [
        'chatTemplate' => "{% if messages[0]['role'] == 'user' or messages[0]['role'] == 'system' %}{{ bos_token }}{% endif %}{% for message in messages %}{{ '' + message['role'] + '\\n' + message['content'] + '' + '\\n' }}{% endfor %}{% if add_generation_prompt %}{{ 'assistant\\n' }}{% elif messages[-1]['role'] == 'assistant' %}{{ eos_token }}{% endif %}",
        'data'          => [
            'messages'              => EXAMPLE_CHAT,
            'bos_token'             => '<bos>',
            'eos_token'             => '<eos>',
    ...
    'cognitivecomputations_dolphin_2_5_mixtral_8x7b' => [
        'chatTemplate' => "{% if not add_generation_prompt is defined %}{% set add_generation_prompt = false %}{% endif %}{% for message in messages %}{{'' + message['role'] + '\\n' + message['content'] + '' + '\\n'}}{% endfor %}{% if add_generation_prompt %}{{ 'assistant\\n' }}{% endif %}",
        'data'          => [
            'messages'  => EXAMPLE_CHAT,
            'bos_token' => '<s>',
            'eos_token' => '</s>',
    ...
    'openchat_openchat_3_5_0106' => [
        'chatTemplate' => "{{ bos_token }}{% for message in messages %}{{ 'GPT4 Correct ' + message['role'].title() + ': ' + message['content'] + ''}}{% endfor %}{% if add_generation_prompt %}{{ 'GPT4 Correct Assistant:' }}{% endif %}",
        'data'          => [
            'messages'              => EXAMPLE_CHAT,
            'bos_token'             => '<s>',
            'eos_token'             => '</s>',
    ...
    'upstage_SOLAR_10_7B_Instruct_v1_0' => [
        'chatTemplate' => "{% for message in messages %}{% if message['role'] == 'system' %}{% if message['content']%}{{'### System:\n' + message['content']+'\n\n'}}{% endif %}{% elif message['role'] == 'user' %}{{'### User:\n' + message['content']+'\n\n'}}{% elif message['role'] == 'assistant' %}{{'### Assistant:\n'  + message['content']}}{% endif %}{% if loop.last and add_generation_prompt %}{{ '### Assistant:\n' }}{% endif %}{% endfor %}",
        'data'          => [
            'messages'              => EXAMPLE_CHAT,
            'bos_token'             => '<s>',
            'eos_token'             => '</s>',
    ...
    'codellama_CodeLlama_70b_Instruct_hf' => [
        'chatTemplate' => "{% if messages[0]['role'] == 'system' %}{% set user_index = 1 %}{% else %}{% set user_index = 0 %}{% endif %}{% for message in messages %}{% if (message['role'] == 'user') != ((loop.index0 + user_index) % 2 == 0) %}{{ raise_exception('Conversation roles must alternate user/assistant/user/assistant/...') }}{% endif %}{% if loop.index0 == 0 %}{{ '<s>' }}{% endif %}{% set content = 'Source: ' + message['role'] + '\\n\\n ' + message['content'] | trim %}{{ content + ' <step> ' }}{% endfor %}{{'Source: assistant\\nDestination: user\\n\\n '}}",
        'data'          => [
            'messages'  => EXAMPLE_CHAT,
            'bos_token' => '<s>',
            'eos_token' => '</s>',
    ...
    'deepseek-ai/deepseek-llm-7b-chat' => [
        'chatTemplate' => "{% if not add_generation_prompt is defined %}{% set add_generation_prompt = false %}{% endif %}{{ bos_token }}{% for message in messages %}{% if message['role'] == 'user' %}{{ 'User: ' + message['content'] + '\n\n' }}{% elif message['role'] == 'assistant' %}{{ 'Assistant: ' + message['content'] + eos_token }}{% elif message['role'] == 'system' %}{{ message['content'] + '\n\n' }}{% endif %}{% endfor %}{% if add_generation_prompt %}{{ 'Assistant:' }}{% endif %}",
        'data'          => [
            'messages'  => EXAMPLE_CHAT,
    ...
    'h2oai/h2o-danube-1.8b-chat' => [
        'chatTemplate' => "{% for message in messages %}{% if message['role'] == 'user' %}{{ '<|prompt|>' + message['content'] + eos_token }}{% elif message['role'] == 'system' %}{{ '<|system|>' + message['content'] + eos_token }}{% elif message['role'] == 'assistant' %}{{ '<|answer|>'  + message['content'] + eos_token }}{% endif %}{% if loop.last and add_generation_prompt %}{{ '<|answer|>' }}{% endif %}{% endfor %}",
        'data'          => [
            'messages'              => EXAMPLE_CHAT,
            'bos_token'             => '<s>',
            'eos_token'             => '</s>',
    ...
    'ericzzz/falcon-rw-1b-chat' => [
        'chatTemplate' => "{% for message in messages %}{% if loop.index > 1 and loop.previtem['role'] != 'assistant' %}{{ ' ' }}{% endif %}{% if message['role'] == 'system' %}{{ '[SYS] ' + message['content'].strip() }}{% elif message['role'] == 'user' %}{{ '[INST] ' + message['content'].strip() }}{% elif message['role'] == 'assistant' %}{{ '[RESP] '  + message['content'] + eos_token }}{% endif %}{% endfor %}{% if add_generation_prompt %}{{ ' [RESP] ' }}{% endif %}",
        'data'          => [
            'messages'              => EXAMPLE_CHAT,
            'bos_token'             => '<|endoftext|>',
            'eos_token'             => '<|endoftext|>',
    ...
    //        'chatTemplate' => "{%- for idx in range(0, messages|length) -%}\n{%- if messages[idx]['role'] == 'user' -%}\n{%- if idx > 1 -%}\n{{- bos_token + '[INST] ' + messages[idx]['content'] + ' [/INST]' -}}\n{%- else -%}\n{{- messages[idx]['content'] + ' [/INST]' -}}\n{%- endif -%}\n{% elif messages[idx]['role'] == 'system' %}\n{{- '[INST] <<SYS>>\\n' + messages[idx]['content'] + '\\n<</SYS>>\\n\\n' -}}\n{%- elif messages[idx]['role'] == 'assistant' -%}\n{{- ' '  + messages[idx]['content'] + ' ' + eos_token -}}\n{% endif %}\n{% endfor %}",
    //        'data' => [
    //            'messages' => EXAMPLE_CHAT,
    //            'bos_token' => "<s>",
    //            'eos_token' => "/s>",
    ...
    'maywell/Synatra-Mixtral-8x7B' => [
        'chatTemplate' => "Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n{% for message in messages %}{% if message['role'] == 'user' %}### Instruction:\n{{ message['content']|trim -}}{% if not loop.last %}{% endif %}\n{% elif message['role'] == 'assistant' %}### Response:\n{{ message['content']|trim -}}{% if not loop.last %}{% endif %}\n{% elif message['role'] == 'system' %}{{ message['content']|trim -}}{% if not loop.last %}{% endif %}\n{% endif %}\n{% endfor %}\n{% if add_generation_prompt and messages[-1]['role'] != 'assistant' %}\n### Response:\n{% endif %}",
        'data'          => [
            'messages'              => EXAMPLE_CHAT,
            'bos_token'             => '<s>',
            'eos_token'             => '</s>',
...
END,
    ...
    //    ],
    'maywell/PiVoT-MoE' => [
        'chatTemplate' => "{{ (messages|selectattr('role', 'equalto', 'system')|list|last).content|trim if (messages|selectattr('role', 'equalto', 'system')|list) else '' }}{% for message in messages %}{% if message['role'] == 'system' %}{{ message['content']|trim }}{% elif message['role'] == 'user' %}### Instruction: {{ message['content']|trim }}{% elif message['role'] == 'assistant' %}### Response: {{ message['content']|trim }}{% elif message['role'] == 'user_context' %}### Input: {{ message['content']|trim }}{% endif %}{% if not loop.last %}\n{% endif %}{% endfor %}{% if add_generation_prompt and messages[-1]['role'] != 'assistant' %}### Response:{% endif %}",
        'data'          => [
            'messages'              => $EXAMPLE_CHAT_WITH_SYSTEM,
            'bos_token'             => '<s>',
            'eos_token'             => '</s>',
